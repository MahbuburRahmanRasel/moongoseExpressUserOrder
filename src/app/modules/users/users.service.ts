...
const updateUser =async (userId:number, userData: any):Promise<IUserOrder|null> => {
...
const deleteUser =async (userId:number):Promise<IUserOrder|null> => {
    const result = await User.findOneAndDelete({userId:userId})
    return result
}


const updateOrder =async (userId:number,  newOrderData: any):Promise<IUserOrder|null> => {
    const result = await User.findOneAndUpdate(
        { 'user.userId': userId },
...
const getTotalPrice = async (userId: string): Promise<number | undefined>  => 
        {
        ...
        const totalPrice = user?.order?.reduce((acc, order) => acc + order.price * order.quantity, 0);
